...
export async function renderServiceDetail(root: HTMLElement, project: api.Project, service: api.McpService) {
  ...
  const codeCmd = `claude mcp add ${service.id} ${mcpUrl} --transport http`;
...
async function renderConfigTab(container: HTMLElement, project: api.Project, service: api.McpService) {
  ...
  if (service.id === "yt-mcp") {
    ...
    const exportCmd = "yt-dlp --cookies-from-browser firefox --cookies ~/yt-cookies-full.txt >/dev/null 2>&1; { head -3 ~/yt-cookies-full.txt; grep -iE '(youtube|google|googlevideo)\\.com' ~/yt-cookies-full.txt; } > ~/yt-cookies.txt && rm ~/yt-cookies-full.txt && echo 'Saved to ~/yt-cookies.txt'";
...
async function showKeyCreatedModal(container: HTMLElement, created: api.CreatedKey, service: api.McpService) {
  ...
  const cliCmd = `claude mcp add ${service.id} https://${subdomain}.${domain}/mcp --transport http -H "Authorization: Bearer ${created.key}"`;
